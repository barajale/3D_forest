...
for k=1:length(files)
    ...
    save('test', 'P');
    create_input;
    QSMs = make_models_parallel('test', 'QSMs_test', 10, inputs);
    [ TreeData , OptModels , OptInputs , OptQSM ] = select_optimum(QSMs, 'cyl_vol_dia10_mean');
    QSMs2 = make_models_parallel( 'test' , 'QSMs test2' , 25, OptInputs );
    ...
    final = strcat('./post_processed/super_big_today/', newStr, datestr, '.mat');
    disp(final);
    save(final, 'OptQSM');
